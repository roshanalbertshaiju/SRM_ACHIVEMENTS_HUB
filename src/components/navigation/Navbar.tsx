  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-3 flex-shrink-0">
        ...
        {/* Primary CTA: + Share Achievement */}
        <button
          onClick={() => setIsAddModalOpen(true)}
          className="px-3.5 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold flex items-center space-x-1.5 transition-all shadow-sm tactile-btn"
        >
                ...
                {primaryTabs.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'p-2.5 rounded-xl text-xs font-bold flex items-center space-x-2 border transition-all text-left',
                        isActive
                ...
                {menuGroups.flatMap((g) => g.items).map((item, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'p-2.5 rounded-xl text-left flex items-center space-x-2 border transition-colors',
      ...
      <nav
        className={cn(
          'fixed bottom-0 left-0 right-0 z-40 h-16 border-t backdrop-blur-2xl flex items-center justify-around px-2 lg:hidden shadow-2xl',
          isLight ? 'bg-white/95 border-slate-200 text-slate-900' : 'bg-[#0B1020]/95 border-white/10 text-slate-100'
        )}
      >
        {primaryTabs.map((tab) => {
          ...
          return (
            <button
              ...
              onClick={() => setActiveScreen(tab.screen)}
              className={cn(
                'flex flex-col items-center justify-center space-y-0.5 py-1 px-3 rounded-xl transition-all',
                isActive
        ...
        <button
          ...
          className="flex flex-col items-center justify-center space-y-0.5 py-1 px-3 text-purple-400 hover:text-purple-300"
        >
          <Palette className="w-5 h-5" />
          <span className="text-[10px] font-semibold">Skins</span>
        </button>
      </nav>
